...
mybubble.controller('BubblesCtrl',['$scope','$http',function($scope,$http) {
     $http.get("http://labnewsapp.herokuapp.com/top10").success(function(data){
       ...
       /*
      $scope.getbub = function (){
        console.log("askcbksbcksbck");
            var diameter = 960,
    format = d3.format(",d"),
...
function classes(root) {
  ...
  function recurse(name, node) {
    if (node) node.forEach(function(child) { recurse(node, child); });
    else classes.push({packageName: Bubbles.topic, className: node, value: node});
  }
